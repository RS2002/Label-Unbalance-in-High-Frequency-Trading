...
class Mamba(nn.Module):
    def __init__(self, input_dim=13, output_dim=1, hidden_dim=768, state_size=16, num_hidden_layers=32, da=128, r=4,
                 norm=False):
        super().__init__()
...
class BERT(nn.Module):
    def __init__(self, input_dim=13, output_dim=1, hidden_dim=768, intermediate_size=3072, num_attention_heads=12,
                 num_hidden_layers=12, da=128, r=4, norm=False):
        super().__init__()
        self.linear_input = nn.Sequential(
            nn.Linear(input_dim, 256),
            nn.ReLU(),
        ...
        self.config = BertConfig(hidden_size=hidden_dim, intermediate_size=intermediate_size,
...
class MLP_ZXM(nn.Module):
    def __init__(self, input_dim, length, hidden_dim, output_dim):
        super(MLP_ZXM, self).__init__()
        self.fc1 = nn.Linear(length * input_dim, hidden_dim)
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size, length * input_dim)
...
if __name__ == '__main__':
    ...
    bert = BERT(input_dim=13, output_dim=1, hidden_dim=64, intermediate_size=64, num_attention_heads=4,
                num_hidden_layers=4, da=128, r=4, norm=True)
